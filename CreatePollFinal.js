...
class CreatePoll extends Component {
    ...
    handleClickOutside = event => {
        ...
        if((event.target && event.target.id) !== (this.questionInput.current && this.questionInput.current.id)) {
    ...
    onAddChoice = () => {
        ...
        if(pollChoices.length == 6) {
          ToastsStore.warning(t('polls.choiceExceedMsg'), 2000);
            ...
            this.setState({pollChoices, selectedChoiceEle: "", selectedChoiceIndex: null});
        }
    }
    ...
    onRemoveChoice = (indx) => {
        ...
        if(pollChoices.length < 3) {
            ...
            this.setState({pollChoices: pollChoices, duplicateIdx: duplicateIdxGrp})
        }
    }

    onCreatePoll = () => {
        const {t, closeModal}=this.props;
        const {pollChoices, pollTitle,singleChoiceSelected,endDate, categoryType, selectedMembers, selectedUnits}=this.state;
        ...
        if(!pollTitle) {
        ...
        } else if(selectedMembers.length == 0 && selectedUnits.length == 0 && categoryType !== "everyone") {
            ...
            selectedMembers.map(mem => participants.push({UserName: mem.UserName}));
            ...
            let payload = {
                ...
                UnitValueParticipants: unitValParticipants.length>0 ? unitValParticipants : []
            };
            this.setState({creating: true});
            new Api(GetAPIHeader(Storage.getAccessToken())).v31.
            createPoll(payload).then(res => {
    ...
    handleOnDragEnd = (param) =>{
        ...
        if(desI !== undefined){
            ...
            this.setState({pollChoices: pollChoices, draggingState: false, draggingChoiceId: null});
        }
    }
    ...
    checkStateChanged = () => {
        ...
        const {pollTitle, singleChoiceSelected, multiChoiceSelected, pollChoices, endDate} = this.state;
        
        if(pollTitle !== "" || !singleChoiceSelected || multiChoiceSelected || pollChoices.length !== 2 || pollChoices[0].text !== "" || pollChoices[1].text !== "" || endDate.format("DD/MM/YYYY") !== moment(new Date(), "DD/MM/YYYY").add(1, 'days').format("DD/MM/YYYY")) {
            this.onCloseModal();
        } else {
            closeModal();
        }
    ...
    onAddMembers = (type, members, units) => {
        this.setState({
            ...
            selectedUnits: units,
            pollAudience: type=="everyone" ? [] : type=="groups" ? units : members,
    ...
    removeSelected(item) {
        ...
        if(item.type == "individuals") {
            let removeIndex = pollAudience.map(mem => mem.UserName).indexOf(item.UserName);
                    ...
                    <div>
                        ...
                        <div style={{ maxHeight:120, overflow: 'scroll'}} >
                            <div className="d-flex flex-wrap">
                                {pollAudience.map((item, indx) => {
                                        return (
                                            <>
                                            <div key={indx} className="app-blue-badge-pill ml-2 py-2 my-1 bg-dark-gray text-black jobcriteria-badge-pill text-left">
                                                <span className="text-small font-weight-bold text-gray">{item.Name}</span> 
                                                <i className="fas fa-times fa-sm text-gray pl-2 pointer" onClick={() => this.removeSelected(item)} />
                                            </div>
                                            </>
                                        )
                                })}
                            </div> 
                            <div className="position-absolute" style={{right: 16, bottom:20}}>
                                <i onClick={e => this.showAllTeamMembersClick(e, false)} className="fa fa-chevron-up p-2 ic_container pointer mr-2" />
                            </div>
                        </div> :
                        <div className="d-flex flex-wrap">
                        {pollAudience.map((item, indx) => {
                        ...
                        {remainingTeamMem.length > 0 && 
                            <div onClick={e => this.showAllTeamMembersClick(e, true)}>
                              <Avatar className="small avatar-class ml-2 mt-1 pointer">{'+' + remainingTeamMem.length}</Avatar>
                            </div>}
                        </div>}
                    </div>
                )
    ...
    render() {
        ...
        const { pollTitle, pollChoices, singleChoiceSelected,openSelectMembersPopup,editMember, editGroup,selectedMembers, selectedUnits,categoryType,pollAudience, multiChoiceSelected, pollQuestionSelected, duplicateIdx, selectedChoiceIndex, selectedChoiceEle, endDate,pollQuestionErrMsg, choiceEmptyErrMsg, choiceDuplicateErrMsg, errChoiceIdx, errEmptyChoiceIdx,draggingState,draggingChoiceIdx,showCloseModal}=this.state;
        ...
        return (
            <Modal 
                ...
                <Modal.Body className="bg-gray p-lg-4 p-md-4 p-3" style={{borderRadius: '8px'}}>
                    <div className="d-flex">
                        <h4 className="font-weight-bold pl-2">{t('polls.newPoll')}</h4>
                        <i className="fas fa-times fa-lg text-gray position-absolute pointer" onClick={this.checkStateChanged} style={{right: "30px"}} ></i>
                    </div>
                    <div className="mt-2 container p-2">
                        <div className="col-md-12">
                            <div className="form-group">
                                <label htmlFor="title" className="normal font-weight-bold" style={{color: "#666"}}>  
                                    {t('polls.pollTitle')}
                                </label>
                                <input
                                    type="text"
                                    ...
                                    onClick={() => this.setState({ pollQuestionSelected: true })}
                                ...
                                <p className="float-right" style={{color: "#666", height: "15px"}}>
                                    {pollQuestionSelected && <small>{pollTitle.length} / {maxDescCount}</small>}
                                </p>
                            </div>
                        </div>
                        <div className="d-flex">
                            <div className="form-group col-md-6 pt-4">
                                <label htmlFor="remindUsersBy" className="normal font-weight-bold" style={{color: "#666"}}>{t('polls.addChoices')}</label>
                            </div>
                            <div className="form-group col-md-6 pt-4 d-flex justify-content-around">
                                <div className="d-flex pointer" onClick={() => {
                                    ...
                                    <label htmlFor="singleChoice"
                                        className={singleChoiceSelected ? "normal font-weight-bold ml-2 pointer text-primary" : "normal font-weight-bold ml-2 pointer text-gray"}>
                                        {t('polls.singleChoice')}
                                    </label>
                                </div>
                                <div className="d-flex pointer" onClick={() => {
                                        this.setState({multiChoiceSelected: true, singleChoiceSelected: false})
                                    }}>
                                    {singleChoiceSelected ? <div className="circle-gray mr-1 mt-1"/> : <div className="circle-primary mr-1 mt-1" />}
                                    <label htmlFor="multipleChoice"
                                    className={singleChoiceSelected ? "normal font-weight-bold ml-2 pointer text-gray" : "normal font-weight-bold ml-2 pointer text-primary"}>{t('polls.multipleChoice')}</label>
                                </div>
                            </div>
                            </div>
                            <DragDropContext onDragStart={(param) => {
                                ...
                                {(provided)=>(
                                <div className="form-group col-md-12" {...provided.droppableProps}
                                        ...
                                        return (
                                            ...
                                            <div className="d-flex align-items-center pt-2" 
                                                ...
                                                <input
                                                    ...
                                                    className={choiceDuplicateErrMsg && duplicateIdx.includes(indx) ? "form-control bg-white form-control-md normal input-field border-danger pl-5" : "form-control poll_border form-control-md normal bg-white input-field pl-5"}
                                                    id={indx}
                                                    name="pollChoice"
                                                    autoComplete="off"
                                                    ...
                                                    onChange={(evt) => this.handleChoiceChange(evt, indx)}
                                                    value={choice.text}
                                                />
                                                {!(indx == 1 || indx == 0 ) &&
                                                    <i className="far fa-times-circle text-gray fa-lg position-absolute pointer" 
                                                    onClick={() => {
                                            ...
                                            {choiceDuplicateErrMsg && duplicateIdx.includes(indx) &&
                                                ...
                                                {indx == pollChoices.length-1 &&
                                                    <div className="pl-4">
                                                        <small className="text-gray ml-2">{t('polls.maxSixChoices')}</small>
                                                    </div>}
                                                </div>
                            ...
                            <div className="col-md-12 d-flex flex-row-reverse justify-content-between">
                                <button className={pollChoices.length>5 ? "py-2 app-blue-badge-pill bg-dark-gray btn-border-radius normal" : "py-2 app-blue-badge-pill blue-thick-border btn-border-radius normal" }
                                            onClick={this.onAddChoice} 
                                            disabled={pollChoices.length>5 ? true : false} 
                                            style={{lineHeight: 'unset'}}>
                                    <span className={pollChoices.length>5 ? "text-gray" : "text-primary"}>{t('common.add')}</span>
                                    <i className={pollChoices.length>5 ? "fa fa-plus fa-lg ml-3 text-gray": "fa fa-plus fa-lg ml-3 text-primary"} aria-hidden="true"></i>
                                </button>
                            ...
                            <div className="col-md-12 d-flex flex-row-reverse">
                                ...
                                <button className={"py-2 app-blue-badge-pill blue-thick-border btn-border-radius normal"}
                                ...
                                <button className={"py-2 app-blue-badge-pill blue-thick-border btn-border-radius normal"}
                                    ...
                                    <i className={"fa fa-plus fa-lg ml-3 text-primary"} aria-hidden="true" />
                                </button>}
                            </div>
                            ...
                            <div>
                            <label htmlFor="remindUsersBy" className="normal font-weight-bold" style={{color: "#666"}}>{t('polls.pollCloseOn')}</label>
                            <span className="small text-gray pl-1 pointer" data-toggle="tooltip" 
                                data-placement="top" title={t('polls.closingPollTooltip')}>{"(?)"}</span>
                            </div>
                                <FilledDatePicker
                                        disablePast={true} 
                                        value={endDate}
                ...
                <Modal.Footer className="pr-4 pb-2" style={{ border: '0px', backgroundColor: '#F9F9F9' }}>
                    <Button onClick={this.onCreatePoll} size="sm" disabled={this.state.creating}>
                        {t("polls.publishPoll")}
                        {this.state.creating &&
                            <span>
